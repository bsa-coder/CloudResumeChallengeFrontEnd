...
if __name__ == '__main__':
    """
    delete_visits_table()
    print("Visits table deleted.")
    """
    
    visit_table = create_visits_table()
    print("Table status:", visit_table.table_status)
    print("the number of items is: ",visit_table.item_count)

    response = visit_table.get_item(Key={'id': 'site_visits'})
    response = update_visit(datetime.now())
    cur_visit = get_record('current_visit')
    print('cur_visit=',cur_visit)
    print("cur_visit['visit_datetime']=",cur_visit['visit_datetime'])
    print("strptime=",datetime.strptime(cur_visit['visit_datetime'], '%Y-%m-%d %H:%M:%S.%f'))

    visit_sum = get_record('site_visits')
    update_visits(datetime.strptime(cur_visit['visit_datetime'], '%Y-%m-%d %H:%M:%S.%f'))
    print(visit_sum)
    visit_sum = get_record('site_visits')
    print(visit_sum)
    update_visits(datetime.strptime(cur_visit['visit_datetime'], '%Y-%m-%d %H:%M:%S.%f'))
    print(visit_sum)
    #print(visit_table.get_item(Key={'id': 'site_visits'}))
    
    """
    visit_resp = put_visit(now(), 0)
    print("Put visit succeeded:")
    pprint(visit_resp, sort_dicts=False)

    try:
        visit = get_visit("current_visit",)
        if visit:
            #movieinfo = JSON.parse(movie)
            print("Get visit succeeded:")
            print(visit)
            pprint(visit, sort_dicts=False)

    except:
        print("Get visit FAILED:")

    update_response = update_visit(time)
    print("Update visit succeeded:")
